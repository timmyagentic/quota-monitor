...
const simplifiedChinese = Object.freeze({
  ...
  privacyStatisticsBody: "可选检查 JSON payload 只包含公开说明的六个字段；它和 D1 原始及聚合数据集都不包含稳定安装 ID 或设备 ID。服务的设计目的不是跨 UTC 日关联安装；Cloudflare 的网络处理在完整政策中单独披露。",
  ...
  privacyPolicyOptOutBody: "关闭报告会立即停止发起新请求，尝试取消任何在途请求，删除本机当天的令牌和成功状态，并抑制同一 UTC 日内重新启用，直到下一个 UTC 日。已经到达服务端的请求仍可能被接收。由此产生或此前已接收的匿名行无法单独定位或删除，因为不存在稳定 ID 或删除句柄；它们遵循上述相同的实时原始行和 D1 Time Travel 保留规则。",
