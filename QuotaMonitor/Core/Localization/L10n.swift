...
enum L10n {
    ...
    // Codex Fast-Mode billing — local Codex traces can tag many turns
    // with Fast or Standard tier. This setting keeps the existing
    // storage key but is now only a fallback for rows whose tier remains
    // unclassified after trace detection.
