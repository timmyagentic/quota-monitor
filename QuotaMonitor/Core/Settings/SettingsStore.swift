...
final class SettingsStore {
    ...
    /// local trace data. Explicit Fast and Standard event tiers are priced
    /// independently of this setting; when ON, only unknown-tier events
    /// for models listed in `CodexFastMode.multipliers` are routed to the
    /// synthetic `<model>-fast` catalog row. Toggling re-runs
