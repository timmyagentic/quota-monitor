...
enum Migrations {
    ...
    static func register(in migrator: inout DatabaseMigrator) {
        ...
        // once per matching event. These indexes retain the prefixes used by
        // the prior timestamp-only indexes, so they replace rather than duplicate
        // those read paths.
        migrator.registerMigration("v16-history-covering-indexes") { db in
            try db.execute(sql: """
                CREATE INDEX IF NOT EXISTS idx_usage_events_history_cover
                ON usage_events(timestamp, value_usd, total_tokens, session_id)
                """)
            try db.execute(sql: """
                CREATE INDEX IF NOT EXISTS idx_usage_events_provider_history_cover
                ON usage_events(
                    provider, timestamp, value_usd, total_tokens, session_id
                )
                """)
            try db.execute(sql: """
