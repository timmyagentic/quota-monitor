...
#### Summary
- Codex Fast Mode costs are now split from Standard usage when local history markers identify them
...
### Changed
- **Codex Fast Mode billing uses detected tiers first.** Codex imports now read local history `fast_mode` / `quick_mode` markers to split Standard, Fast, and unknown usage, while the existing Fast Mode setting only reprices unclassified events.
