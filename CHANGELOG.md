...
- The Codex window widget now matches the menu bar's 5h/7d labels, uses a more polished compact style, and opens a narrower detail card that dismisses naturally.
- Dates and times in Sessions, History, Activity, and Trends now match the language selected inside Quota Monitor.

### Fixed

- **Refined Codex quota widget.** The window widget and menu bar now share the same compact quota labels, while the anchored detail card is narrower and closes after pointer exit or a click elsewhere instead of remaining pinned.
