...
- The Codex window widget now mirrors the menu bar's compact 5h/7d readout without an extra status dot, rests on a barely visible semantic surface that becomes clearer on hover, follows Codex's help button when it is available and otherwise uses a stable fixed slot 150 points from the window's leading edge, and opens a clearly branded, narrower detail card without an unnecessary scroll bar at normal window sizes.
...
- **Refined Codex quota widget.** The window widget now uses the same compact quota text as the menu bar without an extra status dot; its idle state uses a barely visible semantic surface and hairline border for a stable native-control outline, while hover gently strengthens the surface, border, and shadow. Its position follows Codex's discovered help control with clear spacing, while unavailable Accessibility data falls back to the original fixed-slot algorithm with the trailing edge 150 points from the window's leading edge. The anchored detail card uses the native semantic window background, starts with the Quota Monitor name, stays narrow, scrolls only when a short window would otherwise clip content, and closes after pointer exit or a click elsewhere instead of remaining pinned.
