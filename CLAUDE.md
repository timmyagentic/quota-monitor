...
- **Pricing is bundled, effective-dated, and never read from a live catalog.** `PricingService.installBundledCatalog()` restores the app-shipped rows on every database open, then token counts become `value_usd` through `backfillAllValues()`. Vendor price changes must update the bundled current row and add an effective-date period so old usage is not repriced. Three traps baked into the provider-branched SQL: for **Codex**, cached reads and cache writes are disjoint subsets already inside `input_tokens`, so both are subtracted before ordinary input pricing; every Standard/Flex/Fast catalog row and historical period must materialize its cache-write rate (1.25× the corresponding uncached input rate), and event valuation reads that stored rate directly before applying the 2× long-context multiplier above 272K; only `price_source = 'bundled'` rows in the explicit Codex model set may participate, so unsupported legacy rows and bundled Claude/GLM rows stay out of the Codex branch; **reasoning tokens are not added to output** (output already includes them — adding them double-bills). Recorded Priority/Flex preferences select synthetic tier rows; unknown preferences remain Standard, while GPT-5.6 Priority keeps Fast pricing in long context.
