...
struct CodexBillingTierImportTests {
    ...
    func reparsingPreservesStampedBillingTierWhenTraceIsGone() async throws {
        ...
        let turn = "019f0000-0000-7000-8000-0000000000cc"
        let sessionId = "019f0000-0000-7000-8000-0000000000dd"
        let eventEpoch = 1_783_400_000.0
        let eventTs = ISO8601.fractional.string(
            from: Date(timeIntervalSince1970: eventEpoch))
        ...
        try await traceQueue.write { conn in
            try conn.execute(sql: "DELETE FROM logs")
        }
        let noopLine = """
        {"timestamp":"\(eventTs)","type":"event_msg","payload":{"type":"noop"}}

        """
        try (jsonl + noopLine).write(to: rollout, atomically: true, encoding: .utf8)
        _ = try await engine.performScan()
        ...
        #expect(tier == "priority",
                "a re-parse must carry forward a trace-confirmed priority tier")
    }
}
