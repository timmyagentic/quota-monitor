...
struct SessionTitleProjectMetadataTests {
    ...
    @Test("CSV export title falls back to project metadata")
    func csvExportTitleFallsBackToProjectMetadata() async throws {
        let db = try makeDatabase()
        let output = FileManager.default.temporaryDirectory
            .appendingPathComponent("qm-session-metadata-export-\(UUID().uuidString).csv")
            ...
            try conn.execute(sql: """
                INSERT INTO sessions
                  (session_id, root_session_id, parent_session_id, title,
                   ...
                   contains_subagents, created_at, imported_at, provider)
                VALUES
                  ('real-title', 'real-title', NULL,
                   'Split session titles from project metadata',
                   'quota-monitor', '/Volumes/SamsungDisk/Code/quota-monitor',
                   NULL, '2026-06-01T00:00:00Z', '2026-06-01T00:00:00Z',
                   NULL, NULL, 'gpt-5.5', NULL, 0,
                   '2026-06-01T00:00:00Z', '2026-06-01T00:00:00Z', 'codex'),
                  ('project-only', 'project-only', NULL,
                   NULL,
                   'project-name-fallback-demo', '/Volumes/SamsungDisk/Code/project-name-fallback-demo',
                   NULL, '2026-06-01T00:01:00Z', '2026-06-01T00:01:00Z',
                   NULL, NULL, 'gpt-5.5', NULL, 0,
                   '2026-06-01T00:01:00Z', '2026-06-01T00:01:00Z', 'codex')
                """)
            try conn.execute(sql: """
                INSERT INTO usage_events
        ...
        #expect(realTitleFields[6] == "jsonl")
