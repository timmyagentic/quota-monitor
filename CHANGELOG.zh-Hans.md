...
#### Summary
- Codex Fast Mode 费用现在会在本地历史标记能识别时与标准用量分开统计
- Claude 实时配额卡片现在等待刷新时会保留上次数值
- Mac App Store 准备工作现在有本地预检流程，开始账号配置前就能先评估可行性
...
### 变更
- **Codex Fast Mode 计费优先使用已识别档位。** Codex 导入现在会读取本地历史里的 `fast_mode` / `quick_mode` 标记，分开统计标准、Fast 和未知用量；原有 Fast Mode 设置只会重算无法识别档位的事件。
