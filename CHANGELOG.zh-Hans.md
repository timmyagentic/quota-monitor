...
- 当 Codex 由合并后的 ChatGPT 桌面应用提供时，现在也能正常获取实时用量百分比。
- 官网现在会把真实的菜单栏弹窗与 Dashboard 放在首屏一起展示，让用户在下载前就能看懂常驻额度体验。
- 有可用更新时，现在会显示紧凑的蓝色圆形下载图标，不再使用较笨重的文字按钮。
- 官网与双语 README 现在会展示常驻额度读数和点击展开的 Codex/Claude 弹窗，并通过不含真实数据的合成示例说明重置时间、节奏提示、模型独立额度和主动重置卡。
- 完整中英双语的项目首页现在优先展示官网和下载入口，并提供快速语言切换、更清晰的产品介绍、隐私摘要、截图以及独立的 App 和官网开发路径。
- 高级设置现在只保留用户需要的控制项，不再展示内部数据库、CSV 导出和计价管理工具。
- 现在可以用一条 `npx` 命令安装 Quota Monitor，也可以让 AI 编程 Agent 协助完成；熟悉的 DMG 下载方式继续保留。
- 较大的活跃 Codex 历史现在只读取新追加的数据，不再重复读取整个会话，同时保持 Token、额度和费用统计结果一致。
