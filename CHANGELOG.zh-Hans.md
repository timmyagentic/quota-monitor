...
- Codex Fast 自动识别在重扫或复制本地 trace 快照后也更可靠。
...
- **Codex trace 快照会可靠重新打标。** QuotaMonitor 现在会在本地 trace 数据变化时重新执行 Codex 计费档位归因，即使 rollout 文件本身没有变化；复制出来的 trace 快照也会被规范化，确保没有 WAL sidecar 文件时仍能只读打开。
