...
- **仪表盘新增「使用画像」一节。** 仿 CodeX 的使用概览：顶部是五项指标（累计 tokens、单日峰值、最长任务、当前连续天数、最长连续天数），下方是 GitHub 风格的 Token 活跃度热力图，支持每日 / 每周 / 累计三种视图。所有数字都跟随当前 Provider 过滤，并完全由已有的本地历史推导，不额外采集数据、不改数据库结构。
