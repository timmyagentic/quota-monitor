...
- 已授权的 Mac 现在可以直接在 Quota Monitor 内选择私有 Beta 更新，同时不会在 GitHub 上公开 Beta 下载。
- 重要的产品更新现在可以通过一个支持图片与短视频、且可随时重看的“近期新功能”窗口进行介绍。
...
- Dashboard 现在会显示最近 7 个和 30 个本地自然日的加权缓存命中率，并可按所选范围查看每日趋势。
- 高级设置中的“自动检查更新”开关下方不再显示说明文字，界面更简洁。

### 新增

- **私有 Beta 更新通道。** 已授权的 Mac 可用一次性代码登记，在高级设置中切换稳定版与私有 Beta，并通过 Sparkle 获取需鉴权的更新，而 Beta 文件不会进入公开 GitHub Release 或更新源。
- **可重看的近期新功能演示。** 重要版本可以在既有用户下一次主动操作应用时展示一次双语离线导览，支持随包图片、静音视频与封面降级、“减少动态效果”、键盘导航，并在菜单弹窗和设置中保留永久重看入口；全新安装仍直接完成首次设置，不会紧接着再看更新导览。
- **Dashboard 缓存命中率趋势。** 顶部摘要现在会同时显示固定的近 7 天、近 30 天加权结果，并将每日缓存命中率折线整合进现有 Token 柱状图；Provider 与日期筛选会同步更新两处数据。
