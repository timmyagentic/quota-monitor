...
- 现在可以选择在 Codex 活跃窗口内附着每周额度胶囊，悬停查看剩余额度和重置时间，且不会修改 Codex 应用。
...
- **在工作位置直接查看额度。** 通用设置新增 Codex 附着额度胶囊；它复用 Quota Monitor 已有实时快照，跟随前台 Codex 窗口、随 Codex 隐藏、悬停展开详情，同时保持已签名的 Codex 应用完全不变。
