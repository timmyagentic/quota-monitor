...
- 网站 HTTP 请求现在会在静态资源交付前进入 Worker，因此不安全链接会稳定重定向到 HTTPS。
- 有可用更新时，打开 Quota Monitor 后会看到蓝色 `Update` 按钮；macOS 菜单栏文字保持原样，选择“稍后”也不会反复弹窗。

### 变更

- **自适应历史分页。** 历史记录按建立索引的 7 天窗口加载；首屏不足以填满侧边栏时最多自动补载 3 页，之后每次主动向下滚动只加载一页。
- **更安静的常驻更新入口。** 选择“稍后”后，同一版本的自动检查会保持安静，蓝色 `Update` 按钮则会跨重启保留在菜单栏弹窗、Dashboard 工具栏和 Advanced 设置中。
