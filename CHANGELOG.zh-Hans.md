...
- 项目首页现在优先展示官网和下载入口，并用更清晰的产品介绍、隐私摘要、截图以及独立的 App 和官网开发路径说明 Quota Monitor。
- 高级设置现在只保留用户需要的控制项，不再展示内部数据库、CSV 导出和计价管理工具。

### 变更

- **项目 README。** GitHub 项目首页现在可直接进入官网和下载页，以面向用户的语言说明核心能力与数据边界，并明确记录仓库内 `website/` 官网源码及其验证流程。
