...
#### Summary
- Claude 卡片现在会在当前订阅提供 Sonnet-only 周配额时显示对应用量条
- 贡献者现在有一份简洁的仓库指南，说明项目结构和本地检查方式
- 已经选过语言的老用户更新后不再重复进入 Landing Page

### 新增
- **Claude Sonnet-only 配额行。** 当 Anthropic 为当前账号返回 Sonnet-only 周配额窗口时，菜单栏 Claude 卡片现在会显示对应用量条；即使该 scoped 周配额没有自己的 reset 时间，也会沿用总 7 天窗口的 reset；没有该配额的订阅仍保持简洁。
