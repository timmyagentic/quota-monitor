...
- 开启 Codex 侧栏挂件后现在会先静默尝试连接，仅在确实需要时明确提示你手动退出 Codex，再自动打开一次；整个流程不会强制关闭 Codex 或打断你的工作。
- 后台检查更新现在不会打断当前工作：应用内图标可一键开始更新、紧凑显示进度，并只在全部准备完成后提示重新启动。
- 现在可以把 Codex 的 5 小时和每周额度直接显示在账户行内，精致挂件会在页面切换、主题变化及后续重启 Codex 后继续稳定挂载。
...
- **更安全的 Codex 挂件启用流程。** 开启原生额度挂件后会先尝试仅附着连接，持续显示当前状态；只有连接失败才提示你手动退出 Codex，并仅为这次明确启用授权一次自动重开，持久化启用和后续普通退出都不会再次打开 Codex。
- **更温和的一键更新。** 自动检查发现新版本时只显示现有应用内更新图标；点击后立即开始下载与校验，以紧凑状态反馈进度，并仅在更新准备完成后弹出重新启动提示，同时“立即检查”仍会显示明确结果。
- **可靠的 Codex 侧栏额度。** 可选集成现在使用 Opsail 成熟的 Codex 挂载生命周期，在账户行内显示 5 小时和每周额度、跟随主题变量、通过实时额度桥接刷新，并可在页面切换或 Codex 重启后恢复，关闭功能时也会完整移除挂件。
