...
- Codex 侧栏挂件现在采用清晰的用户主动启动流程：需要时由你手动退出 Codex，再从设置中明确点击并用挂件模式打开。
...
- **显式启动 Codex 挂件。** 原生额度挂件会先静默尝试连接；如果需要重新启动，则只提示你自行退出，并等待你另行点击“打开 Codex 并启用挂件”，Quota Monitor 不会自动退出或重新打开 Codex。
