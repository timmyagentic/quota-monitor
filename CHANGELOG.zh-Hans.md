...
- Codex 费用估算现在会区分记录为 Standard、Fast 与 Flex 的 turn，并避免把子代理历史里重放的父会话用量重复计算。

### 变更

- **Codex 服务档位按 turn 估算。** 近期 Codex turn 会按记录的 Standard、Fast 或 Flex 偏好估算；Flex 使用其公开的低价档位，现有开关仍只为旧版或未标记用量提供 Fast 回退。
